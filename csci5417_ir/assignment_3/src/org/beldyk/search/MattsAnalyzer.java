...
// cat medical.txt |perl -e 'while(<>){map{$f{$_} += 1;}(split(/[^\w]+/,$_));}; map{print "$_ $f{$_}\n";} (sort {$f{$b} <=> $f{$a}} keys %f);' |perl -ne '/(\w+)\s/;print "\"$1\", ";'
	...
	private String [] myStopWords = {
			"of", "the", "and", "in", "to", "S", "a", "with", "U", "P", "A", 
			"T", "was", "M", "I", "were", "patients", "for", "The", "ARTICLE", 
			"JOURNAL", "Human", "by", "that", "W", "1", "is", "or", "2", "J", 
			"from", "on", "than", "0", "t", "Support", "Gov", "5", "3", "as", "be", 
			"at", "an", "ME", "Male", "Female", "4", "6", "not", "are", "cells", "PA", 
			"after", "In", "had", "H", "Non", "this", "Adult"	
			};
